...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("alefsbits");

    if !plugin_dir.exists() {
        eprintln!("Plugin alefsbits not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("Logic", "alefsbitsLogic");
    build.define("modelLogic", "modelalefsbitsLogic");
    build.define("LogicWidget", "alefsbitsLogicWidget");

    // Filter-out list
    let filter_out: Vec<String> = vec![
        "alefsbits/src/plugin.cpp".to_string(),
    ];

    // Source files

    // Glob alefsbits/src/**/*.cpp|cc|c (recursive)
    ...
    collect_sources(&plugins_dir.join("alefsbits/src"), &filter_out, &plugins_dir, &mut build, 0);

    build.compile("cardinal_plugin_alefsbits");
}
