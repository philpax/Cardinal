...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("BogaudioModules-helper");

    if !plugin_dir.exists() {
        eprintln!("Plugin BogaudioModules-helper not found (submodule not initialized?), skipping");
        return;
    }
    ...
    // Source files
    build.file(plugins_dir.join("BogaudioModules-helper/BogaudioModules-helper.cpp"));

    build.compile("cardinal_plugin_bogaudiomodules_helper");
}
