...
fn main() {
    ...
    // Glob BogaudioModules/src/**/*.cpp|cc|c (recursive)
    ...
    collect_sources(&plugins_dir.join("BogaudioModules/src"), &filter_out, &plugins_dir, &mut build, 0);

    // Glob BogaudioModules/src/dsp/**/*.cpp|cc|c (recursive)
    fn collect_sources(dir: &std::path::Path, filter_out: &[String], plugins_dir: &std::path::Path, build: &mut cc::Build, depth: u32) {
        if depth > 5 || !dir.exists() { return; }
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
    ...
    collect_sources(&plugins_dir.join("BogaudioModules/src/dsp"), &filter_out, &plugins_dir, &mut build, 0);

    // Glob BogaudioModules/src/dsp/filters/**/*.cpp|cc|c (recursive)
    fn collect_sources(dir: &std::path::Path, filter_out: &[String], plugins_dir: &std::path::Path, build: &mut cc::Build, depth: u32) {
        if depth > 5 || !dir.exists() { return; }
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
    ...
    collect_sources(&plugins_dir.join("BogaudioModules/src/dsp/filters"), &filter_out, &plugins_dir, &mut build, 0);
