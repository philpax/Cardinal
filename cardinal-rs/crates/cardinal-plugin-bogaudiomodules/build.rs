...
fn main() {
    ...
    // Glob BogaudioModules/src/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("BogaudioModules/src")) {
    ...
    // Glob BogaudioModules/src/dsp/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("BogaudioModules/src/dsp")) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().map_or(true, |e| e != "cpp" && e != "cc" && e != "c") { continue; }
            let rel = path.strip_prefix(&plugins_dir).unwrap().to_str().unwrap().to_string();
    ...
    // Glob BogaudioModules/src/dsp/filters/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("BogaudioModules/src/dsp/filters")) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().map_or(true, |e| e != "cpp" && e != "cc" && e != "c") { continue; }
            let rel = path.strip_prefix(&plugins_dir).unwrap().to_str().unwrap().to_string();
