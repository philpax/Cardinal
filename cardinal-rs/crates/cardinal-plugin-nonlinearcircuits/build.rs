...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("nonlinearcircuits");

    if !plugin_dir.exists() {
        eprintln!("Plugin nonlinearcircuits not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__nonlinearcircuits");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "nonlinearcircuits/src/NLC.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("nonlinearcircuits/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-nonlinearcircuits/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_nonlinearcircuits");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_nonlinearcircuits");
}
