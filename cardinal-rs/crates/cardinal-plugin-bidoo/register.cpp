...
extern "C" void cardinal_register_Bidoo() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Bidoo");
    const std::string mf = rack::asset::pluginManifest("Bidoo");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Bidoo: %s\n", mf.c_str());
        delete p;
        pluginInstance__Bidoo = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Bidoo\n");
        delete p;
        pluginInstance__Bidoo = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Bidoo: %s\n", e.what());
    }
