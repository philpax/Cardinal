...
extern "C" void cardinal_register_Orbits() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Orbits");
    const std::string mf = rack::asset::pluginManifest("Orbits");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Orbits: %s\n", mf.c_str());
        delete p;
        pluginInstance__Orbits = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Orbits\n");
        delete p;
        pluginInstance__Orbits = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Orbits: %s\n", e.what());
    }
