...
extern "C" void cardinal_register_8Mode() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("8Mode");
    const std::string mf = rack::asset::pluginManifest("8Mode");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for 8Mode: %s\n", mf.c_str());
        delete p;
        pluginInstance__8Mode = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for 8Mode\n");
        delete p;
        pluginInstance__8Mode = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for 8Mode: %s\n", e.what());
    }
