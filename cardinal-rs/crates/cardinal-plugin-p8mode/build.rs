...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("8Mode");

    if !plugin_dir.exists() {
        eprintln!("Plugin 8Mode not found (submodule not initialized?), skipping");
        return;
    }
    ...
    // Glob 8Mode/src/**/*.cpp|cc|c (recursive)
    ...
    collect_sources(&plugins_dir.join("8Mode/src"), &filter_out, &plugins_dir, &mut build, 0);
