...
extern "C" void cardinal_register_RebelTech() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("RebelTech");
    const std::string mf = rack::asset::pluginManifest("RebelTech");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for RebelTech: %s\n", mf.c_str());
        delete p;
        pluginInstance__RebelTech = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for RebelTech\n");
        delete p;
        pluginInstance__RebelTech = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for RebelTech: %s\n", e.what());
    }
