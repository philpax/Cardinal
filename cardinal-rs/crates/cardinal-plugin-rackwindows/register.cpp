// Auto-generated — registration function for rackwindows
// Renames init() only in the included file, not globally
#define init init__rackwindows
#include "rackwindows/src/plugin.cpp"
#undef init
...
extern "C" void cardinal_register_rackwindows() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("rackwindows");
    const std::string mf = rack::asset::pluginManifest("rackwindows");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for rackwindows: %s\n", mf.c_str());
        delete p;
        pluginInstance__rackwindows = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for rackwindows\n");
        delete p;
        pluginInstance__rackwindows = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for rackwindows: %s\n", e.what());
    }
