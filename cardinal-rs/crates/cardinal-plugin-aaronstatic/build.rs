...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("AaronStatic");

    if !plugin_dir.exists() {
        eprintln!("Plugin AaronStatic not found (submodule not initialized?), skipping");
        return;
    }
    ...
    // Glob AaronStatic/src/**/*.cpp|cc|c (recursive)
    ...
    collect_sources(&plugins_dir.join("AaronStatic/src"), &filter_out, &plugins_dir, &mut build, 0);
