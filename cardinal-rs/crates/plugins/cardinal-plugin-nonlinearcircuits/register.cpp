...
extern "C" void cardinal_register_nonlinearcircuits() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("nonlinearcircuits");
    const std::string mf = rack::asset::pluginManifest("nonlinearcircuits");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for nonlinearcircuits: %s\n", mf.c_str());
        delete p;
        pluginInstance__nonlinearcircuits = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for nonlinearcircuits\n");
        delete p;
        pluginInstance__nonlinearcircuits = nullptr;
        return;
    }
    ...
    try {
        init__nonlinearcircuits(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for nonlinearcircuits: %s\n", e.what());
    }
