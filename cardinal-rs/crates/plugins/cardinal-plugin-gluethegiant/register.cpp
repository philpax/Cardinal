...
extern "C" void cardinal_register_GlueTheGiant() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("GlueTheGiant");
    const std::string mf = rack::asset::pluginManifest("GlueTheGiant");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for GlueTheGiant: %s\n", mf.c_str());
        delete p;
        pluginInstance__GlueTheGiant = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for GlueTheGiant\n");
        delete p;
        pluginInstance__GlueTheGiant = nullptr;
        return;
    }
    ...
    try {
        init__GlueTheGiant(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for GlueTheGiant: %s\n", e.what());
    }
