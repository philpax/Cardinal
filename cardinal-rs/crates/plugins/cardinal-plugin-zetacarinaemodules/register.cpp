...
extern "C" void cardinal_register_ZetaCarinaeModules() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("ZetaCarinaeModules");
    const std::string mf = rack::asset::pluginManifest("ZetaCarinaeModules");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for ZetaCarinaeModules: %s\n", mf.c_str());
        delete p;
        pluginInstance__ZetaCarinaeModules = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for ZetaCarinaeModules\n");
        delete p;
        pluginInstance__ZetaCarinaeModules = nullptr;
        return;
    }
    ...
    try {
        init__ZetaCarinaeModules(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for ZetaCarinaeModules: %s\n", e.what());
    }
