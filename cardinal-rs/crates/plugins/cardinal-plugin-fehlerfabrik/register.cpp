...
extern "C" void cardinal_register_FehlerFabrik() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("FehlerFabrik");
    const std::string mf = rack::asset::pluginManifest("FehlerFabrik");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for FehlerFabrik: %s\n", mf.c_str());
        delete p;
        pluginInstance__FehlerFabrik = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for FehlerFabrik\n");
        delete p;
        pluginInstance__FehlerFabrik = nullptr;
        return;
    }
    ...
    try {
        init__FehlerFabrik(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for FehlerFabrik: %s\n", e.what());
    }
