...
extern "C" void cardinal_register_EnigmaCurry() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("EnigmaCurry");
    const std::string mf = rack::asset::pluginManifest("EnigmaCurry");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for EnigmaCurry: %s\n", mf.c_str());
        delete p;
        pluginInstance__EnigmaCurry = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for EnigmaCurry\n");
        delete p;
        pluginInstance__EnigmaCurry = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for EnigmaCurry: %s\n", e.what());
    }
