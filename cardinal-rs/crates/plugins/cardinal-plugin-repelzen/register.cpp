// Auto-generated — registration function for repelzen
// Renames init() only in the included file, not globally
#define init init__repelzen
#include "repelzen/src/repelzen.cpp"
#undef init
...
extern "C" void cardinal_register_repelzen() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("repelzen");
    const std::string mf = rack::asset::pluginManifest("repelzen");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for repelzen: %s\n", mf.c_str());
        delete p;
        pluginInstance__repelzen = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for repelzen\n");
        delete p;
        pluginInstance__repelzen = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for repelzen: %s\n", e.what());
    }
