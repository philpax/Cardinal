...
extern "C" void cardinal_register_LilacLoop() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("LilacLoop");
    const std::string mf = rack::asset::pluginManifest("LilacLoop");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for LilacLoop: %s\n", mf.c_str());
        delete p;
        pluginInstance__LilacLoop = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for LilacLoop\n");
        delete p;
        pluginInstance__LilacLoop = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for LilacLoop: %s\n", e.what());
    }
