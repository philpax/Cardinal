...
extern "C" void cardinal_register_unless_modules() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("unless_modules");
    const std::string mf = rack::asset::pluginManifest("unless_modules");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for unless_modules: %s\n", mf.c_str());
        delete p;
        pluginInstance__unless_modules = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for unless_modules\n");
        delete p;
        pluginInstance__unless_modules = nullptr;
        return;
    }
    ...
    try {
        init__unless_modules(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for unless_modules: %s\n", e.what());
    }
