// Auto-generated — registration function for LyraeModules
// Renames init() only in the included file, not globally
#define init init__Lyrae
#include "LyraeModules/src/plugin.cpp"
#undef init
...
extern "C" void cardinal_register_Lyrae() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("LyraeModules");
    const std::string mf = rack::asset::pluginManifest("LyraeModules");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for LyraeModules: %s\n", mf.c_str());
        delete p;
        pluginInstance__Lyrae = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for LyraeModules\n");
        delete p;
        pluginInstance__Lyrae = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for LyraeModules: %s\n", e.what());
    }
