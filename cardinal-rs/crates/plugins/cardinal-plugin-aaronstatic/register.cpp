...
extern "C" void cardinal_register_AaronStatic() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("AaronStatic");
    const std::string mf = rack::asset::pluginManifest("AaronStatic");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for AaronStatic: %s\n", mf.c_str());
        delete p;
        pluginInstance__AaronStatic = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for AaronStatic\n");
        delete p;
        pluginInstance__AaronStatic = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for AaronStatic: %s\n", e.what());
    }
