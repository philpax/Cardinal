...
extern "C" void cardinal_register_MSM() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("MSM");
    const std::string mf = rack::asset::pluginManifest("MSM");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for MSM: %s\n", mf.c_str());
        delete p;
        pluginInstance__MSM = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for MSM\n");
        delete p;
        pluginInstance__MSM = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for MSM: %s\n", e.what());
    }
