...
extern "C" void cardinal_register_Prism() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Prism");
    const std::string mf = rack::asset::pluginManifest("Prism");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Prism: %s\n", mf.c_str());
        delete p;
        pluginInstance__Prism = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Prism\n");
        delete p;
        pluginInstance__Prism = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Prism: %s\n", e.what());
    }
