...
extern "C" void cardinal_register_admiral() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("admiral");
    const std::string mf = rack::asset::pluginManifest("admiral");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for admiral: %s\n", mf.c_str());
        delete p;
        pluginInstance__admiral = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for admiral\n");
        delete p;
        pluginInstance__admiral = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for admiral: %s\n", e.what());
    }
