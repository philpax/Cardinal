...
extern "C" void cardinal_register_CatroModulo() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("CatroModulo");
    const std::string mf = rack::asset::pluginManifest("CatroModulo");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for CatroModulo: %s\n", mf.c_str());
        delete p;
        pluginInstance__CatroModulo = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for CatroModulo\n");
        delete p;
        pluginInstance__CatroModulo = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for CatroModulo: %s\n", e.what());
    }
