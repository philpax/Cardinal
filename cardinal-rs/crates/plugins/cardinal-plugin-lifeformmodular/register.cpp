...
extern "C" void cardinal_register_LifeFormModular() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("LifeFormModular");
    const std::string mf = rack::asset::pluginManifest("LifeFormModular");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for LifeFormModular: %s\n", mf.c_str());
        delete p;
        pluginInstance__LifeFormModular = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for LifeFormModular\n");
        delete p;
        pluginInstance__LifeFormModular = nullptr;
        return;
    }
    ...
    try {
        init__LifeFormModular(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for LifeFormModular: %s\n", e.what());
    }
