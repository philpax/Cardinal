...
extern "C" void cardinal_register_MUS_X() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("MUS-X");
    const std::string mf = rack::asset::pluginManifest("MUS-X");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for MUS-X: %s\n", mf.c_str());
        delete p;
        pluginInstance__MUS_X = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for MUS-X\n");
        delete p;
        pluginInstance__MUS_X = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for MUS-X: %s\n", e.what());
    }
