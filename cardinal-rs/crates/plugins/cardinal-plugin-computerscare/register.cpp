...
extern "C" void cardinal_register_Computerscare() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Computerscare");
    const std::string mf = rack::asset::pluginManifest("Computerscare");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Computerscare: %s\n", mf.c_str());
        delete p;
        pluginInstance__Computerscare = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Computerscare\n");
        delete p;
        pluginInstance__Computerscare = nullptr;
        return;
    }
    ...
    try {
        init__Computerscare(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for Computerscare: %s\n", e.what());
    }
