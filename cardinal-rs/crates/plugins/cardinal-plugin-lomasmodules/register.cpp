// Auto-generated — registration function for LomasModules
// Renames init() only in the included file, not globally
#define init init__Lomas
#include "LomasModules/src/plugin.cpp"
#undef init
...
extern "C" void cardinal_register_Lomas() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("LomasModules");
    const std::string mf = rack::asset::pluginManifest("LomasModules");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for LomasModules: %s\n", mf.c_str());
        delete p;
        pluginInstance__Lomas = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for LomasModules\n");
        delete p;
        pluginInstance__Lomas = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for LomasModules: %s\n", e.what());
    }
