...
extern "C" void cardinal_register_ChowDSP() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("ChowDSP");
    const std::string mf = rack::asset::pluginManifest("ChowDSP");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for ChowDSP: %s\n", mf.c_str());
        delete p;
        pluginInstance__ChowDSP = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for ChowDSP\n");
        delete p;
        pluginInstance__ChowDSP = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for ChowDSP: %s\n", e.what());
    }
