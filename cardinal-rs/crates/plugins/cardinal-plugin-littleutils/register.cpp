...
extern "C" void cardinal_register_LittleUtils() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("LittleUtils");
    const std::string mf = rack::asset::pluginManifest("LittleUtils");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for LittleUtils: %s\n", mf.c_str());
        delete p;
        pluginInstance__LittleUtils = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for LittleUtils\n");
        delete p;
        pluginInstance__LittleUtils = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for LittleUtils: %s\n", e.what());
    }
