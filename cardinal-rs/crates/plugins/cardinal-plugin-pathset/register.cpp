...
extern "C" void cardinal_register_PathSet() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("PathSet");
    const std::string mf = rack::asset::pluginManifest("PathSet");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for PathSet: %s\n", mf.c_str());
        delete p;
        pluginInstance__PathSet = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for PathSet\n");
        delete p;
        pluginInstance__PathSet = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for PathSet: %s\n", e.what());
    }
