...
extern "C" void cardinal_register_Meander() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Meander");
    const std::string mf = rack::asset::pluginManifest("Meander");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Meander: %s\n", mf.c_str());
        delete p;
        pluginInstance__Meander = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Meander\n");
        delete p;
        pluginInstance__Meander = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Meander: %s\n", e.what());
    }
