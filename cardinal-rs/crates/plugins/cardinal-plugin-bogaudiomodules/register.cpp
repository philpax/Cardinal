...
extern "C" void cardinal_register_BogaudioModules() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("BogaudioModules");
    const std::string mf = rack::asset::pluginManifest("BogaudioModules");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for BogaudioModules: %s\n", mf.c_str());
        delete p;
        pluginInstance__BogaudioModules = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for BogaudioModules\n");
        delete p;
        pluginInstance__BogaudioModules = nullptr;
        return;
    }
    ...
    try {
        init__BogaudioModules(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for BogaudioModules: %s\n", e.what());
    }
