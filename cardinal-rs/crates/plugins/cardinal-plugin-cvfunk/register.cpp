...
extern "C" void cardinal_register_CVfunk() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("CVfunk");
    const std::string mf = rack::asset::pluginManifest("CVfunk");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for CVfunk: %s\n", mf.c_str());
        delete p;
        pluginInstance__CVfunk = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for CVfunk\n");
        delete p;
        pluginInstance__CVfunk = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for CVfunk: %s\n", e.what());
    }
