...
extern "C" void cardinal_register_ZZC() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("ZZC");
    const std::string mf = rack::asset::pluginManifest("ZZC");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for ZZC: %s\n", mf.c_str());
        delete p;
        pluginInstance__ZZC = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for ZZC\n");
        delete p;
        pluginInstance__ZZC = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for ZZC: %s\n", e.what());
    }
