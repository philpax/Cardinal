...
extern "C" void cardinal_register_AnimatedCircuits() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("AnimatedCircuits");
    const std::string mf = rack::asset::pluginManifest("AnimatedCircuits");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for AnimatedCircuits: %s\n", mf.c_str());
        delete p;
        pluginInstance__AnimatedCircuits = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for AnimatedCircuits\n");
        delete p;
        pluginInstance__AnimatedCircuits = nullptr;
        return;
    }
    ...
    try {
        init__AnimatedCircuits(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for AnimatedCircuits: %s\n", e.what());
    }
