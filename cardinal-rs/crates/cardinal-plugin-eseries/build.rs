...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("ESeries");

    if !plugin_dir.exists() {
        eprintln!("Plugin ESeries not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-eseries/register.cpp"));
