...
extern "C" void cardinal_register_ESeries() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("ESeries");
    const std::string mf = rack::asset::pluginManifest("ESeries");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for ESeries: %s\n", mf.c_str());
        delete p;
        pluginInstance__ESeries = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for ESeries\n");
        delete p;
        pluginInstance__ESeries = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for ESeries: %s\n", e.what());
    }
