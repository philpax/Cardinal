...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("Autinn");

    if !plugin_dir.exists() {
        eprintln!("Plugin Autinn not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__Autinn");
    build.define("Chord", "AutinnChord");
    build.define("modelChord", "modelAutinnChord");
    build.define("ChordWidget", "AutinnChordWidget");
    build.define("Vibrato", "AutinnVibrato");
    build.define("modelVibrato", "modelAutinnVibrato");
    build.define("VibratoWidget", "AutinnVibratoWidget");
    build.define("Snare", "AutinnSnare");
    build.define("modelSnare", "modelAutinnSnare");
    build.define("SnareWidget", "AutinnSnareWidget");
    build.define("Scope", "AutinnScope");
    build.define("modelScope", "modelAutinnScope");
    build.define("ScopeWidget", "AutinnScopeWidget");
    build.define("ScopeDisplay", "AutinnScopeDisplay");
    build.define("modelScopeDisplay", "modelAutinnScopeDisplay");
    build.define("ScopeDisplayWidget", "AutinnScopeDisplayWidget");
    build.define("DCBlocker", "AutinnDCBlocker");
    build.define("modelDCBlocker", "modelAutinnDCBlocker");
    build.define("DCBlockerWidget", "AutinnDCBlockerWidget");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "Autinn/src/Autinn.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("Autinn/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-autinn/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_autinn");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_autinn");
}
