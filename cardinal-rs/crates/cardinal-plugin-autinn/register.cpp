// Auto-generated — registration function for Autinn
// Renames init() only in the included file, not globally
#define init init__Autinn
#include "Autinn/src/Autinn.cpp"
#undef init
...
extern "C" void cardinal_register_Autinn() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Autinn");
    const std::string mf = rack::asset::pluginManifest("Autinn");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Autinn: %s\n", mf.c_str());
        delete p;
        pluginInstance__Autinn = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Autinn\n");
        delete p;
        pluginInstance__Autinn = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Autinn: %s\n", e.what());
    }
