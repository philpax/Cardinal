...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("eightfold");

    if !plugin_dir.exists() {
        eprintln!("Plugin eightfold not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__eightfold");
    build.define("Comparator", "eightfoldComparator");
    build.define("modelComparator", "modeleightfoldComparator");
    build.define("ComparatorWidget", "eightfoldComparatorWidget");
    build.define("comparator", "eightfoldcomparator");
    build.define("modelcomparator", "modeleightfoldcomparator");
    build.define("comparatorWidget", "eightfoldcomparatorWidget");
    ...
    let _filter_out: Vec<String> = vec![
        "eightfold/src/plugin.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("eightfold/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-eightfold/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_eightfold");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_eightfold");
}
