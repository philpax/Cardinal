// Auto-generated — registration function for eightfold
// Renames init() only in the included file, not globally
#define init init__eightfold
#include "eightfold/src/plugin.cpp"
#undef init
...
extern "C" void cardinal_register_eightfold() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("eightfold");
    const std::string mf = rack::asset::pluginManifest("eightfold");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for eightfold: %s\n", mf.c_str());
        delete p;
        pluginInstance__eightfold = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for eightfold\n");
        delete p;
        pluginInstance__eightfold = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for eightfold: %s\n", e.what());
    }
