...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("MockbaModular");

    if !plugin_dir.exists() {
        eprintln!("Plugin MockbaModular not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("Comparator", "MockbaModularComparator");
    build.define("modelComparator", "modelMockbaModularComparator");
    ...
    // Glob MockbaModular/src/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("MockbaModular/src")) {
