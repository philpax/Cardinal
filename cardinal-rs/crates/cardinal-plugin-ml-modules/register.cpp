...
extern "C" void cardinal_register_ML() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("ML_modules");
    const std::string mf = rack::asset::pluginManifest("ML_modules");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for ML_modules: %s\n", mf.c_str());
        delete p;
        pluginInstance__ML = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for ML_modules\n");
        delete p;
        pluginInstance__ML = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for ML_modules: %s\n", e.what());
    }
