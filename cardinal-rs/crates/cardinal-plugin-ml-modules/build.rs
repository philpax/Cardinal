...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("ML_modules");

    if !plugin_dir.exists() {
        eprintln!("Plugin ML_modules not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__ML");
    ...
    collect_sources(&plugins_dir.join("ML_modules/src"), &_filter_out, &plugins_dir, &mut build, 0);
    build.file(plugins_dir.join("ML_modules/freeverb/revmodel.cpp"));

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-ml-modules/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_ml_modules");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_ml_modules");
}
