...
extern "C" void cardinal_register_ihtsyn() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("ihtsyn");
    const std::string mf = rack::asset::pluginManifest("ihtsyn");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for ihtsyn: %s\n", mf.c_str());
        delete p;
        pluginInstance__ihtsyn = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for ihtsyn\n");
        delete p;
        pluginInstance__ihtsyn = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for ihtsyn: %s\n", e.what());
    }
