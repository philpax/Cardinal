...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("surgext-helper");

    if !plugin_dir.exists() {
        eprintln!("Plugin surgext-helper not found (submodule not initialized?), skipping");
        return;
    }
    ...
    // Source files
    build.file(plugins_dir.join("surgext-helper/surgext-helper.cpp"));

    build.compile("cardinal_plugin_surgext_helper");
}
