...
extern "C" void cardinal_register_HamptonHarmonics() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("HamptonHarmonics");
    const std::string mf = rack::asset::pluginManifest("HamptonHarmonics");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for HamptonHarmonics: %s\n", mf.c_str());
        delete p;
        pluginInstance__HamptonHarmonics = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for HamptonHarmonics\n");
        delete p;
        pluginInstance__HamptonHarmonics = nullptr;
        return;
    }
    ...
    try {
        init__HamptonHarmonics(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for HamptonHarmonics: %s\n", e.what());
    }
