...
extern "C" void cardinal_register_AS() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("AS");
    const std::string mf = rack::asset::pluginManifest("AS");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for AS: %s\n", mf.c_str());
        delete p;
        pluginInstance__AS = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for AS\n");
        delete p;
        pluginInstance__AS = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for AS: %s\n", e.what());
    }
