...
extern "C" void cardinal_register_sapphire() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Sapphire");
    const std::string mf = rack::asset::pluginManifest("Sapphire");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Sapphire: %s\n", mf.c_str());
        delete p;
        pluginInstance__sapphire = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Sapphire\n");
        delete p;
        pluginInstance__sapphire = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Sapphire: %s\n", e.what());
    }
