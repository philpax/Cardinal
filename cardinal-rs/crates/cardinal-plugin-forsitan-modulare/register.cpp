...
extern "C" void cardinal_register_forsitan() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("forsitan-modulare");
    const std::string mf = rack::asset::pluginManifest("forsitan-modulare");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for forsitan-modulare: %s\n", mf.c_str());
        delete p;
        pluginInstance__forsitan = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for forsitan-modulare\n");
        delete p;
        pluginInstance__forsitan = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for forsitan-modulare: %s\n", e.what());
    }
