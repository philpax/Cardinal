// Auto-generated — registration function for Befaco
// Renames init() only in the included file, not globally
#define init init__Befaco
#include "Befaco/src/plugin.cpp"
#undef init
...
extern "C" void cardinal_register_Befaco() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Befaco");
    const std::string mf = rack::asset::pluginManifest("Befaco");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Befaco: %s\n", mf.c_str());
        delete p;
        pluginInstance__Befaco = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Befaco\n");
        delete p;
        pluginInstance__Befaco = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Befaco: %s\n", e.what());
    }
