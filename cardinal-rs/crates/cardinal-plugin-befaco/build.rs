...
fn main() {
    ...
    // Glob Befaco/src/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("Befaco/src")) {
    ...
    // Glob Befaco/src/noise-plethora/*/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("Befaco/src/noise-plethora/*")) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().map_or(true, |e| e != "cpp" && e != "cc" && e != "c") { continue; }
            let rel = path.strip_prefix(&plugins_dir).unwrap().to_str().unwrap().to_string();
