...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("Befaco");

    if !plugin_dir.exists() {
        eprintln!("Plugin Befaco not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("Mixer", "BefacoMixer");
    build.define("modelMixer", "modelBefacoMixer");
    build.define("MixerWidget", "BefacoMixerWidget");
    build.define("chowdsp", "Befacochowdsp");
    build.define("modelchowdsp", "modelBefacochowdsp");
    build.define("chowdspWidget", "BefacochowdspWidget");
    build.define("Burst", "BefacoBurst");
    build.define("modelBurst", "modelBefacoBurst");
    build.define("BurstWidget", "BefacoBurstWidget");
    build.define("RecordButton", "BefacoRecordButton");
    build.define("modelRecordButton", "modelBefacoRecordButton");
    build.define("RecordButtonWidget", "BefacoRecordButtonWidget");
    build.define("Mixer2", "BefacoMixer2");
    build.define("modelMixer2", "modelBefacoMixer2");
    build.define("Mixer2Widget", "BefacoMixer2Widget");
    build.define("Slew", "BefacoSlew");
    build.define("modelSlew", "modelBefacoSlew");
    build.define("SlewWidget", "BefacoSlewWidget");
    ...
    // Glob Befaco/src/**/*.cpp|cc|c (recursive)
    ...
    collect_sources(&plugins_dir.join("Befaco/src"), &filter_out, &plugins_dir, &mut build, 0);

    // Glob Befaco/src/noise-plethora/*/**/*.cpp|cc|c (recursive)
    fn collect_sources(dir: &std::path::Path, filter_out: &[String], plugins_dir: &std::path::Path, build: &mut cc::Build, depth: u32) {
        if depth > 5 || !dir.exists() { return; }
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
    ...
    collect_sources(&plugins_dir.join("Befaco/src/noise-plethora/*"), &filter_out, &plugins_dir, &mut build, 0);
