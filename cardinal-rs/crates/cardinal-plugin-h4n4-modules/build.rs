...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("h4n4-modules");

    if !plugin_dir.exists() {
        eprintln!("Plugin h4n4-modules not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__H4N4");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "h4n4-modules/src/plugin.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("h4n4-modules/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-h4n4-modules/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_h4n4_modules");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_h4n4_modules");
}
