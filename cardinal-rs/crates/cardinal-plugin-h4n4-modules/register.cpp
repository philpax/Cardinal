...
extern "C" void cardinal_register_H4N4() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("h4n4-modules");
    const std::string mf = rack::asset::pluginManifest("h4n4-modules");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for h4n4-modules: %s\n", mf.c_str());
        delete p;
        pluginInstance__H4N4 = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for h4n4-modules\n");
        delete p;
        pluginInstance__H4N4 = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for h4n4-modules: %s\n", e.what());
    }
