...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("LittleUtils");

    if !plugin_dir.exists() {
        eprintln!("Plugin LittleUtils not found (submodule not initialized?), skipping");
        return;
    }
    ...
    // Glob LittleUtils/src/**/*.cpp|cc|c (recursive)
    ...
    collect_sources(&plugins_dir.join("LittleUtils/src"), &filter_out, &plugins_dir, &mut build, 0);
