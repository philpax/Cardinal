...
fn main() {
    ...
    let _filter_out: Vec<String> = vec![
        "WSTD-Drums/src/WSTD_Drums.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("WSTD-Drums/deps"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("WSTD-Drums/deps/SynthDevKit/src"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("WSTD-Drums/src"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("WSTD-Drums/src/controller"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("WSTD-Drums/src/model"), &_filter_out, &plugins_dir, &mut build, 0);
    ...
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-wstd-drums/init_wrapper.cpp"));
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_wstd_drums");
}
