...
extern "C" void cardinal_register_WSTD_Drums() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("WSTD-Drums");
    const std::string mf = rack::asset::pluginManifest("WSTD-Drums");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for WSTD-Drums: %s\n", mf.c_str());
        delete p;
        pluginInstance__WSTD_Drums = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for WSTD-Drums\n");
        delete p;
        pluginInstance__WSTD_Drums = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for WSTD-Drums: %s\n", e.what());
    }
