...
extern "C" void cardinal_register_ImpromptuModular() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("ImpromptuModular");
    const std::string mf = rack::asset::pluginManifest("ImpromptuModular");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for ImpromptuModular: %s\n", mf.c_str());
        delete p;
        pluginInstance__ImpromptuModular = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for ImpromptuModular\n");
        delete p;
        pluginInstance__ImpromptuModular = nullptr;
        return;
    }
    ...
    try {
        init__ImpromptuModular(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for ImpromptuModular: %s\n", e.what());
    }
