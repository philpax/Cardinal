...
fn main() {
    ...
    // Glob ImpromptuModular/src/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("ImpromptuModular/src")) {
    ...
    // Glob ImpromptuModular/src/comp/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("ImpromptuModular/src/comp")) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().map_or(true, |e| e != "cpp" && e != "cc" && e != "c") { continue; }
            let rel = path.strip_prefix(&plugins_dir).unwrap().to_str().unwrap().to_string();
