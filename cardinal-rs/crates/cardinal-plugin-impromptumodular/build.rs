...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("ImpromptuModular");

    if !plugin_dir.exists() {
        eprintln!("Plugin ImpromptuModular not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__ImpromptuModular");
    build.define("RefreshCounter", "ImpromptuModularRefreshCounter");
    build.define("modelRefreshCounter", "modelImpromptuModularRefreshCounter");
    build.define("RefreshCounterWidget", "ImpromptuModularRefreshCounterWidget");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "ImpromptuModular/src/ImpromptuModular.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("ImpromptuModular/src"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("ImpromptuModular/src/comp"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-impromptumodular/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_impromptumodular");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_impromptumodular");
}
