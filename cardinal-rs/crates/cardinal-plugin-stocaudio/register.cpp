...
extern "C" void cardinal_register_stocaudio() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("stocaudio");
    const std::string mf = rack::asset::pluginManifest("stocaudio");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for stocaudio: %s\n", mf.c_str());
        delete p;
        pluginInstance__stocaudio = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for stocaudio\n");
        delete p;
        pluginInstance__stocaudio = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for stocaudio: %s\n", e.what());
    }
