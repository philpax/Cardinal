...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("stocaudio");

    if !plugin_dir.exists() {
        eprintln!("Plugin stocaudio not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__stocaudio");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "stocaudio/src/plugin.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("stocaudio/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-stocaudio/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_stocaudio");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_stocaudio");
}
