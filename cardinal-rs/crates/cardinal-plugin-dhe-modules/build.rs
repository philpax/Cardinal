...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("DHE-Modules");

    if !plugin_dir.exists() {
        eprintln!("Plugin DHE-Modules not found (submodule not initialized?), skipping");
        return;
    }
    ...
    let filter_out: Vec<String> = vec![
        "DHE-Modules/src/plugin.cpp".to_string(),
    ];

    // Source files


    build.compile("cardinal_plugin_dhe_modules");
}
