...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("repelzen");

    if !plugin_dir.exists() {
        eprintln!("Plugin repelzen not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("Mixer", "repelzenMixer");
    build.define("modelMixer", "modelrepelzenMixer");
    build.define("MixerWidget", "repelzenMixerWidget");
    build.define("Werner", "repelzenWerner");
    build.define("modelWerner", "modelrepelzenWerner");
    build.define("WernerWidget", "repelzenWernerWidget");
    build.define("tanh_pade", "repelzentanh_pade");
    build.define("modeltanh_pade", "modelrepelzentanh_pade");
    build.define("tanh_padeWidget", "repelzentanh_padeWidget");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "repelzen/src/repelzen.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("repelzen/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-repelzen/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_repelzen");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_repelzen");
}
