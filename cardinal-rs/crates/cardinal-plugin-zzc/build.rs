...
fn main() {
    ...
    // Glob ZZC/src/**/*.cpp|cc|c (recursive)
    ...
    collect_sources(&plugins_dir.join("ZZC/src"), &filter_out, &plugins_dir, &mut build, 0);

    // Glob ZZC/src/Phaseque/**/*.cpp|cc|c (recursive)
    fn collect_sources(dir: &std::path::Path, filter_out: &[String], plugins_dir: &std::path::Path, build: &mut cc::Build, depth: u32) {
        if depth > 5 || !dir.exists() { return; }
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
    ...
    collect_sources(&plugins_dir.join("ZZC/src/Phaseque"), &filter_out, &plugins_dir, &mut build, 0);
