...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("ZZC");

    if !plugin_dir.exists() {
        eprintln!("Plugin ZZC not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("LowFrequencyOscillator", "ZZCLowFrequencyOscillator");
    build.define("modelLowFrequencyOscillator", "modelZZCLowFrequencyOscillator");
    build.define("LowFrequencyOscillatorWidget", "ZZCLowFrequencyOscillatorWidget");
    build.define("LedLight", "ZZCLedLight");
    build.define("modelLedLight", "modelZZCLedLight");
    build.define("LedLightWidget", "ZZCLedLightWidget");
    build.define("Step", "ZZCStep");
    build.define("modelStep", "modelZZCStep");
    build.define("StepWidget", "ZZCStepWidget");
    build.define("PolyModeValueItem", "ZZCPolyModeValueItem");
    build.define("modelPolyModeValueItem", "modelZZCPolyModeValueItem");
    build.define("PolyModeValueItemWidget", "ZZCPolyModeValueItemWidget");
    build.define("PolyModeItem", "ZZCPolyModeItem");
    build.define("modelPolyModeItem", "modelZZCPolyModeItem");
    build.define("PolyModeItemWidget", "ZZCPolyModeItemWidget");
    ...
    collect_sources(&plugins_dir.join("ZZC/src"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("ZZC/src/Phaseque"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-zzc/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_zzc");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_zzc");
}
