...
extern "C" void cardinal_register_Mog() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Mog");
    const std::string mf = rack::asset::pluginManifest("Mog");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Mog: %s\n", mf.c_str());
        delete p;
        pluginInstance__Mog = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Mog\n");
        delete p;
        pluginInstance__Mog = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Mog: %s\n", e.what());
    }
