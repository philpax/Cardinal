...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("Mog");

    if !plugin_dir.exists() {
        eprintln!("Plugin Mog not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__Mog");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "Mog/src/plugin.cpp".to_string(),
    ];
    ...
    build.file(plugins_dir.join("Mog/src/Network.cpp"));
    build.file(plugins_dir.join("Mog/src/Nexus.cpp"));

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-mog/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_mog");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_mog");
}
