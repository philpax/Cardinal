...
fn main() {
    ...
    // Glob rcm-modules/src/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("rcm-modules/src")) {
    ...
    // Glob rcm-modules/src/PianoRoll/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("rcm-modules/src/PianoRoll")) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().map_or(true, |e| e != "cpp" && e != "cc" && e != "c") { continue; }
            let rel = path.strip_prefix(&plugins_dir).unwrap().to_str().unwrap().to_string();
