...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("rcm-modules");

    if !plugin_dir.exists() {
        eprintln!("Plugin rcm-modules not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__RCM");
    build.define("init", "init__RCM");

    // Filter-out list
    let filter_out: Vec<String> = vec![
        "rcm-modules/src/plugin.cpp".to_string(),
    ];

    // Source files

    // Glob rcm-modules/src/**/*.cpp|cc|c (recursive)
    ...
    collect_sources(&plugins_dir.join("rcm-modules/src"), &filter_out, &plugins_dir, &mut build, 0);

    // Glob rcm-modules/src/PianoRoll/**/*.cpp|cc|c (recursive)
    fn collect_sources(dir: &std::path::Path, filter_out: &[String], plugins_dir: &std::path::Path, build: &mut cc::Build, depth: u32) {
        if depth > 5 || !dir.exists() { return; }
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
    ...
    collect_sources(&plugins_dir.join("rcm-modules/src/PianoRoll"), &filter_out, &plugins_dir, &mut build, 0);

    build.compile("cardinal_plugin_rcm_modules");
}
