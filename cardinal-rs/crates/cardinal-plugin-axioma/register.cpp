...
extern "C" void cardinal_register_Axioma() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Axioma");
    const std::string mf = rack::asset::pluginManifest("Axioma");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Axioma: %s\n", mf.c_str());
        delete p;
        pluginInstance__Axioma = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Axioma\n");
        delete p;
        pluginInstance__Axioma = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Axioma: %s\n", e.what());
    }
