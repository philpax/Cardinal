...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("CatroModulo");

    if !plugin_dir.exists() {
        eprintln!("Plugin CatroModulo not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("NumDisplayWidget", "CatroModuloNumDisplayWidget");
    build.define("modelNumDisplayWidget", "modelCatroModuloNumDisplayWidget");
    build.define("NumDisplayWidgetWidget", "CatroModuloNumDisplayWidgetWidget");
    ...
    collect_sources(&plugins_dir.join("CatroModulo/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-catromodulo/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_catromodulo");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_catromodulo");
}
