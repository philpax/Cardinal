...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("ImpromptuModularDark");

    if !plugin_dir.exists() {
        eprintln!("Plugin ImpromptuModularDark not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.file(plugins_dir.join("ImpromptuModularDark/PanelTheme.cpp"));

    // Init wrapper (renames init() only for the plugin registration file)

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_impromptumodulardark");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_impromptumodulardark");
}
