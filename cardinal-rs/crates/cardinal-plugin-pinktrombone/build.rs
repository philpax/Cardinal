...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("PinkTrombone");

    if !plugin_dir.exists() {
        eprintln!("Plugin PinkTrombone not found (submodule not initialized?), skipping");
        return;
    }
    ...
    // Glob PinkTrombone/src/**/*.cpp|cc|c (recursive)
    ...
    collect_sources(&plugins_dir.join("PinkTrombone/src"), &filter_out, &plugins_dir, &mut build, 0);

    // Glob PinkTrombone/src/PinkTrombone/**/*.cpp|cc|c (recursive)
    fn collect_sources(dir: &std::path::Path, filter_out: &[String], plugins_dir: &std::path::Path, build: &mut cc::Build, depth: u32) {
        if depth > 5 || !dir.exists() { return; }
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
    ...
    collect_sources(&plugins_dir.join("PinkTrombone/src/PinkTrombone"), &filter_out, &plugins_dir, &mut build, 0);
