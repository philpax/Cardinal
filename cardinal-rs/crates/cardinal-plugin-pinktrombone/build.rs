...
fn main() {
    ...
    // Glob PinkTrombone/src/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("PinkTrombone/src")) {
    ...
    // Glob PinkTrombone/src/PinkTrombone/*.cpp
    if let Ok(entries) = std::fs::read_dir(plugins_dir.join("PinkTrombone/src/PinkTrombone")) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().map_or(true, |e| e != "cpp" && e != "cc" && e != "c") { continue; }
            let rel = path.strip_prefix(&plugins_dir).unwrap().to_str().unwrap().to_string();
