...
extern "C" void cardinal_register_PinkTrombone() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("PinkTrombone");
    const std::string mf = rack::asset::pluginManifest("PinkTrombone");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for PinkTrombone: %s\n", mf.c_str());
        delete p;
        pluginInstance__PinkTrombone = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for PinkTrombone\n");
        delete p;
        pluginInstance__PinkTrombone = nullptr;
        return;
    }
    ...
    try {
        init__PinkTrombone(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for PinkTrombone: %s\n", e.what());
    }
