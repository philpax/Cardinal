...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("MindMeldModular");

    if !plugin_dir.exists() {
        eprintln!("Plugin MindMeldModular not found (submodule not initialized?), skipping");
        return;
    }
    ...
    collect_sources(&plugins_dir.join("MindMeldModular/src"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("MindMeldModular/src/EqMaster"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("MindMeldModular/src/MixMaster"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("MindMeldModular/src/PatchSet"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("MindMeldModular/src/ShapeMaster"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("MindMeldModular/src/Utilities"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("MindMeldModular/src/comp"), &_filter_out, &plugins_dir, &mut build, 0);
