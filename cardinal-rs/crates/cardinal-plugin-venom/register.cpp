// Auto-generated — registration function for Venom
// Renames init() only in the included file, not globally
#define init init__Venom
#include "Venom/src/plugin.cpp"
#undef init
...
extern "C" void cardinal_register_Venom() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Venom");
    const std::string mf = rack::asset::pluginManifest("Venom");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Venom: %s\n", mf.c_str());
        delete p;
        pluginInstance__Venom = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Venom\n");
        delete p;
        pluginInstance__Venom = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Venom: %s\n", e.what());
    }
