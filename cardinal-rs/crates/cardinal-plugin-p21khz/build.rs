...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("21kHz");

    if !plugin_dir.exists() {
        eprintln!("Plugin 21kHz not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__21kHz");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "21kHz/src/21kHz.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("21kHz/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-p21khz/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_p21khz");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_p21khz");
}
