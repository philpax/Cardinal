...
extern "C" void cardinal_register_21kHz() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("21kHz");
    const std::string mf = rack::asset::pluginManifest("21kHz");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for 21kHz: %s\n", mf.c_str());
        delete p;
        pluginInstance__21kHz = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for 21kHz\n");
        delete p;
        pluginInstance__21kHz = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for 21kHz: %s\n", e.what());
    }
