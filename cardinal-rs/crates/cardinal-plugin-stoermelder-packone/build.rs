...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("stoermelder-packone");

    if !plugin_dir.exists() {
        eprintln!("Plugin stoermelder-packone not found (submodule not initialized?), skipping");
        return;
    }
    ...
    let _filter_out: Vec<String> = vec![
        "$(STOERMELDER_PACKONE_IGNORED:%=stoermelder-packone/src/%.cpp)".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("stoermelder-packone/src"), &_filter_out, &plugins_dir, &mut build, 0);
