...
extern "C" void cardinal_register_PdArray() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("PdArray");
    const std::string mf = rack::asset::pluginManifest("PdArray");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for PdArray: %s\n", mf.c_str());
        delete p;
        pluginInstance__PdArray = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for PdArray\n");
        delete p;
        pluginInstance__PdArray = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for PdArray: %s\n", e.what());
    }
