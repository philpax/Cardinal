...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("LifeFormModular");

    if !plugin_dir.exists() {
        eprintln!("Plugin LifeFormModular not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("MS", "LifeFormModularMS");
    build.define("modelMS", "modelLifeFormModularMS");
    build.define("MSWidget", "LifeFormModularMSWidget");
    build.define("__ct_base", "LifeFormModular__ct_base");
    build.define("model__ct_base", "modelLifeFormModular__ct_base");
    build.define("__ct_baseWidget", "LifeFormModular__ct_baseWidget");
    build.define("__ct_comp", "LifeFormModular__ct_comp");
    build.define("model__ct_comp", "modelLifeFormModular__ct_comp");
    build.define("__ct_compWidget", "LifeFormModular__ct_compWidget");
    ...
    collect_sources(&plugins_dir.join("LifeFormModular/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-lifeformmodular/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_lifeformmodular");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_lifeformmodular");
}
