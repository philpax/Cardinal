...
extern "C" void cardinal_register_dBiz() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("dBiz");
    const std::string mf = rack::asset::pluginManifest("dBiz");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for dBiz: %s\n", mf.c_str());
        delete p;
        pluginInstance__dBiz = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for dBiz\n");
        delete p;
        pluginInstance__dBiz = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for dBiz: %s\n", e.what());
    }
