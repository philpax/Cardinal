...
extern "C" void cardinal_register_WhatTheRack() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("WhatTheRack");
    const std::string mf = rack::asset::pluginManifest("WhatTheRack");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for WhatTheRack: %s\n", mf.c_str());
        delete p;
        pluginInstance__WhatTheRack = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for WhatTheRack\n");
        delete p;
        pluginInstance__WhatTheRack = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for WhatTheRack: %s\n", e.what());
    }
