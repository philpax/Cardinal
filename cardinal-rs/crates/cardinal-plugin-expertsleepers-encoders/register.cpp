...
extern "C" void cardinal_register_ExpertSleepersEncoders() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("ExpertSleepers-Encoders");
    const std::string mf = rack::asset::pluginManifest("ExpertSleepers-Encoders");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for ExpertSleepers-Encoders: %s\n", mf.c_str());
        delete p;
        pluginInstance__ExpertSleepersEncoders = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for ExpertSleepers-Encoders\n");
        delete p;
        pluginInstance__ExpertSleepersEncoders = nullptr;
        return;
    }
    ...
    try {
        init__ExpertSleepersEncoders(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for ExpertSleepers-Encoders: %s\n", e.what());
    }
