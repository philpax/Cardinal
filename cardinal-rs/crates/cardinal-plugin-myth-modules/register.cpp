...
extern "C" void cardinal_register_myth_modules() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("myth-modules");
    const std::string mf = rack::asset::pluginManifest("myth-modules");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for myth-modules: %s\n", mf.c_str());
        delete p;
        pluginInstance__myth_modules = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for myth-modules\n");
        delete p;
        pluginInstance__myth_modules = nullptr;
        return;
    }
    ...
    try {
        init__myth_modules(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for myth-modules: %s\n", e.what());
    }
