...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("Biset");

    if !plugin_dir.exists() {
        eprintln!("Plugin Biset not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("ButtonTrigger", "BisetButtonTrigger");
    build.define("modelButtonTrigger", "modelBisetButtonTrigger");
    build.define("ButtonTriggerWidget", "BisetButtonTriggerWidget");
    build.define("Clock", "BisetClock");
    build.define("modelClock", "modelBisetClock");
    build.define("ClockWidget", "BisetClockWidget");
    build.define("Tree", "BisetTree");
    build.define("modelTree", "modelBisetTree");
    build.define("TreeWidget", "BisetTreeWidget");
    build.define("TreeDisplay", "BisetTreeDisplay");
    build.define("modelTreeDisplay", "modelBisetTreeDisplay");
    build.define("TreeDisplayWidget", "BisetTreeDisplayWidget");
    ...
    collect_sources(&plugins_dir.join("Biset/src"), &_filter_out, &plugins_dir, &mut build, 0);
    collect_sources(&plugins_dir.join("Biset/src/*"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-biset/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_biset");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_biset");
}
