...
extern "C" void cardinal_register_Biset() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Biset");
    const std::string mf = rack::asset::pluginManifest("Biset");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Biset: %s\n", mf.c_str());
        delete p;
        pluginInstance__Biset = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Biset\n");
        delete p;
        pluginInstance__Biset = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Biset: %s\n", e.what());
    }
