...
extern "C" void cardinal_register_Algoritmarte() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Algoritmarte");
    const std::string mf = rack::asset::pluginManifest("Algoritmarte");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Algoritmarte: %s\n", mf.c_str());
        delete p;
        pluginInstance__Algoritmarte = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Algoritmarte\n");
        delete p;
        pluginInstance__Algoritmarte = nullptr;
        return;
    }
    ...
    try {
        init__Algoritmarte(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for Algoritmarte: %s\n", e.what());
    }
