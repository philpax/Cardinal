...
extern "C" void cardinal_register_Extratone() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Extratone");
    const std::string mf = rack::asset::pluginManifest("Extratone");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Extratone: %s\n", mf.c_str());
        delete p;
        pluginInstance__Extratone = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Extratone\n");
        delete p;
        pluginInstance__Extratone = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Extratone: %s\n", e.what());
    }
