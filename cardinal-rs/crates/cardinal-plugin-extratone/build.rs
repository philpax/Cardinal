...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("Extratone");

    if !plugin_dir.exists() {
        eprintln!("Plugin Extratone not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("pluginInstance", "pluginInstance__Extratone");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "Extratone/src/plugin.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("Extratone/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-extratone/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_extratone");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_extratone");
}
