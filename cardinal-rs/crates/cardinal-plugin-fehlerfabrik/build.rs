...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("FehlerFabrik");

    if !plugin_dir.exists() {
        eprintln!("Plugin FehlerFabrik not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("Sequencer", "FehlerFabrikSequencer");
    build.define("modelSequencer", "modelFehlerFabrikSequencer");
    build.define("SequencerWidget", "FehlerFabrikSequencerWidget");
    build.define("SlewLimiter", "FehlerFabrikSlewLimiter");
    build.define("modelSlewLimiter", "modelFehlerFabrikSlewLimiter");
    build.define("SlewLimiterWidget", "FehlerFabrikSlewLimiterWidget");
    build.define("SimpleDelay", "FehlerFabrikSimpleDelay");
    build.define("modelSimpleDelay", "modelFehlerFabrikSimpleDelay");
    build.define("SimpleDelayWidget", "FehlerFabrikSimpleDelayWidget");
    ...
    collect_sources(&plugins_dir.join("FehlerFabrik/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-fehlerfabrik/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_fehlerfabrik");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_fehlerfabrik");
}
