...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("AmalgamatedHarmonics");

    if !plugin_dir.exists() {
        eprintln!("Plugin AmalgamatedHarmonics not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("EvenVCO", "AmalgamatedHarmonicsEvenVCO");
    build.define("modelEvenVCO", "modelAmalgamatedHarmonicsEvenVCO");
    build.define("EvenVCOWidget", "AmalgamatedHarmonicsEvenVCOWidget");
    build.define("LowFrequencyOscillator", "AmalgamatedHarmonicsLowFrequencyOscillator");
    build.define("modelLowFrequencyOscillator", "modelAmalgamatedHarmonicsLowFrequencyOscillator");
    build.define("LowFrequencyOscillatorWidget", "AmalgamatedHarmonicsLowFrequencyOscillatorWidget");
    build.define("Pattern", "AmalgamatedHarmonicsPattern");
    build.define("modelPattern", "modelAmalgamatedHarmonicsPattern");
    build.define("PatternWidget", "AmalgamatedHarmonicsPatternWidget");
    build.define("PinkNoiseGenerator", "AmalgamatedHarmonicsPinkNoiseGenerator");
    build.define("modelPinkNoiseGenerator", "modelAmalgamatedHarmonicsPinkNoiseGenerator");
    build.define("PinkNoiseGeneratorWidget", "AmalgamatedHarmonicsPinkNoiseGeneratorWidget");
    build.define("RedNoiseGenerator", "AmalgamatedHarmonicsRedNoiseGenerator");
    build.define("modelRedNoiseGenerator", "modelAmalgamatedHarmonicsRedNoiseGenerator");
    build.define("RedNoiseGeneratorWidget", "AmalgamatedHarmonicsRedNoiseGeneratorWidget");
    build.define("bogaudio", "AmalgamatedHarmonicsbogaudio");
    build.define("modelbogaudio", "modelAmalgamatedHarmonicsbogaudio");
    build.define("bogaudioWidget", "AmalgamatedHarmonicsbogaudioWidget");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "AmalgamatedHarmonics/src/AH.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("AmalgamatedHarmonics/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-amalgamatedharmonics/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_amalgamatedharmonics");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_amalgamatedharmonics");
}
