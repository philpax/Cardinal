...
extern "C" void cardinal_register_AmalgamatedHarmonics() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("AmalgamatedHarmonics");
    const std::string mf = rack::asset::pluginManifest("AmalgamatedHarmonics");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for AmalgamatedHarmonics: %s\n", mf.c_str());
        delete p;
        pluginInstance__AmalgamatedHarmonics = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for AmalgamatedHarmonics\n");
        delete p;
        pluginInstance__AmalgamatedHarmonics = nullptr;
        return;
    }
    ...
    try {
        init__AmalgamatedHarmonics(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for AmalgamatedHarmonics: %s\n", e.what());
    }
