...
fn main() {
    ...
    collect_sources(&plugins_dir.join("MSM/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-msm/init_wrapper.cpp"));
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_msm");
}
