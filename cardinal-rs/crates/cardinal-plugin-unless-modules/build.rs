...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("unless_modules");

    if !plugin_dir.exists() {
        eprintln!("Plugin unless_modules not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("sign", "unless_modulessign");
    build.define("modelsign", "modelunless_modulessign");
    build.define("signWidget", "unless_modulessignWidget");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "unless_modules/src/unless.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("unless_modules/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-unless-modules/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_unless_modules");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_unless_modules");
}
