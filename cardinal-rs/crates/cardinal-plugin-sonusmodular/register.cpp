// Auto-generated — registration function for sonusmodular
// Renames init() only in the included file, not globally
#define init init__sonusmodular
#include "sonusmodular/src/sonusmodular.cpp"
#undef init
...
extern "C" void cardinal_register_sonusmodular() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("sonusmodular");
    const std::string mf = rack::asset::pluginManifest("sonusmodular");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for sonusmodular: %s\n", mf.c_str());
        delete p;
        pluginInstance__sonusmodular = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for sonusmodular\n");
        delete p;
        pluginInstance__sonusmodular = nullptr;
        return;
    }
    ...
    try {
        init__sonusmodular(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for sonusmodular: %s\n", e.what());
    }
