// Auto-generated — registration function for Fundamental
// Renames init() only in the included file, not globally
#define init init__Fundamental
#include "Fundamental/src/plugin.cpp"
#undef init
...
extern "C" void cardinal_register_Fundamental() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("Fundamental");
    const std::string mf = rack::asset::pluginManifest("Fundamental");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for Fundamental: %s\n", mf.c_str());
        delete p;
        pluginInstance__Fundamental = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for Fundamental\n");
        delete p;
        pluginInstance__Fundamental = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for Fundamental: %s\n", e.what());
    }
