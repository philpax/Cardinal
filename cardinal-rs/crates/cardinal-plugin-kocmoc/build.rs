...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("kocmoc");

    if !plugin_dir.exists() {
        eprintln!("Plugin kocmoc not found (submodule not initialized?), skipping");
        return;
    }
    ...
    build.define("Phasor", "kocmocPhasor");
    build.define("modelPhasor", "modelkocmocPhasor");
    build.define("PhasorWidget", "kocmocPhasorWidget");
    build.define("__ct_base", "kocmoc__ct_base");
    build.define("model__ct_base", "modelkocmoc__ct_base");
    build.define("__ct_baseWidget", "kocmoc__ct_baseWidget");
    build.define("__ct_comp", "kocmoc__ct_comp");
    build.define("model__ct_comp", "modelkocmoc__ct_comp");
    build.define("__ct_compWidget", "kocmoc__ct_compWidget");

    // Filter-out list
    let _filter_out: Vec<String> = vec![
        "kocmoc/src/plugin.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("kocmoc/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-kocmoc/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_kocmoc");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_kocmoc");
}
