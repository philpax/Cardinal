...
extern "C" void cardinal_register_kocmoc() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("kocmoc");
    const std::string mf = rack::asset::pluginManifest("kocmoc");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for kocmoc: %s\n", mf.c_str());
        delete p;
        pluginInstance__kocmoc = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for kocmoc\n");
        delete p;
        pluginInstance__kocmoc = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for kocmoc: %s\n", e.what());
    }
