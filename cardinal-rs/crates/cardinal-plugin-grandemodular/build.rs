...
fn main() {
    ...
    let plugin_dir = plugins_dir.join("GrandeModular");

    if !plugin_dir.exists() {
        eprintln!("Plugin GrandeModular not found (submodule not initialized?), skipping");
        return;
    }
    ...
    let _filter_out: Vec<String> = vec![
        "GrandeModular/src/plugin.cpp".to_string(),
    ];
    ...
    collect_sources(&plugins_dir.join("GrandeModular/src"), &_filter_out, &plugins_dir, &mut build, 0);

    // Init wrapper (renames init() only for the plugin registration file)
    build.file(std::path::Path::new("/home/user/Cardinal/cardinal-rs/crates/cardinal-plugin-grandemodular/init_wrapper.cpp"));

    build.cargo_metadata(false);
    build.compile("cardinal_plugin_grandemodular");
    ...
    println!("cargo:rustc-link-lib=static:+whole-archive=cardinal_plugin_grandemodular");
}
