...
extern "C" void cardinal_register_GrandeModular() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("GrandeModular");
    const std::string mf = rack::asset::pluginManifest("GrandeModular");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for GrandeModular: %s\n", mf.c_str());
        delete p;
        pluginInstance__GrandeModular = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for GrandeModular\n");
        delete p;
        pluginInstance__GrandeModular = nullptr;
        return;
    }
    ...
    try {
        init__GrandeModular(p);
    } catch (const std::exception& e) {
        fprintf(stderr, "cardinal: WARNING: init failed for GrandeModular: %s\n", e.what());
    }
