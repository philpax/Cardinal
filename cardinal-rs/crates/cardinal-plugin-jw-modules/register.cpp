...
extern "C" void cardinal_register_JW() {
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("JW-Modules");
    const std::string mf = rack::asset::pluginManifest("JW-Modules");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {
        fprintf(stderr, "cardinal: WARNING: manifest not found for JW-Modules: %s\n", mf.c_str());
        delete p;
        pluginInstance__JW = nullptr;
        return;
    }
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for JW-Modules\n");
        delete p;
        pluginInstance__JW = nullptr;
        return;
    }
    ...
    try {
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for JW-Modules: %s\n", e.what());
    }
