...
extern "C" void cardinal_register_{safe_name}() {{
    ...
    // Load manifest
    p->path = rack::asset::pluginPath("{vendor}");
    const std::string mf = rack::asset::pluginManifest("{vendor}");
    FILE* file = std::fopen(mf.c_str(), "r");
    if (!file) {{
        fprintf(stderr, "cardinal: WARNING: manifest not found for {vendor}: %s\\n", mf.c_str());
        delete p;
        pluginInstance__{safe_name} = nullptr;
        return;
    }}
    json_error_t error;
    ...
    std::fclose(file);
    if (!rootJ) {{
        fprintf(stderr, "cardinal: WARNING: failed to parse manifest for {vendor}\\n");
        delete p;
        pluginInstance__{safe_name} = nullptr;
        return;
    }}
    ...
    try {{
        ...
        fprintf(stderr, "cardinal: WARNING: init failed for {vendor}: %s\\n", e.what());
    }}
